...
with(plotdata, {
  plot(numdate, global_active_power, ylab="Global Active Power", type="l", xaxt="n", xlab="")
  xpoints <- c(min(numdate),median(numdate),max(numdate))
  axis(1, at=c(xpoints),labels=c('Thu','Fri','Sat'))
  
  plot(numdate, sub_metering_1, ylab="Energy sub metering", type="l", xaxt="n", xlab="")
  ...
  plot(numdate, voltage, ylab="Voltage", type="l", xaxt="n", xlab="datetime")
  xpoints <- c(min(numdate),median(numdate),max(numdate))
  axis(1, at=c(xpoints),labels=c('Thu','Fri','Sat'))
  
  plot(numdate, global_reactive_power, ylab="Global_reactive_power", type="l", xaxt="n", xlab="datetime") 
  xpoints <- c(min(numdate),median(numdate),max(numdate))
  axis(1, at=c(xpoints),labels=c('Thu','Fri','Sat'))
})
